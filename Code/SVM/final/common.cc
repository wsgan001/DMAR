...
void get_harr(const vector<double> &X, double *Haar)
{
	vector<double> x;
	x.clear();
	for (int i = 0; i < X.size(); ++ i)
	...
	while (k > 1)
	{
		...
		for (int i = 0; i < k; ++ i)
		{
			ret[i] = (x[i << 1] + x[(i << 1) + 1]) / c;
			ret[i + k] = (x[i << 1] - x[(i << 1) + 1]) / c;
		}
		for (int i = 0; i < (k << 1); ++ i)
			x[i] = ret[i];
	}
...
struct FeatureType XYZ(vector<double> &x, vector<double> &y, vector<double> &z, vector<double> &time)
{
	...
	get_harr(x, ret.Xhaar);
	get_harr(y, ret.Yhaar);
	get_harr(z, ret.Zhaar);
...
void print(int &index, const struct FeatureType &x)
{
	...
	*/
	...
	for (int i = 0; i < HAAR; ++ i)
		printf(" %d:%.4lf", ++ index, x.Xhaar[i]);

	for (int i = 0; i < HAAR; ++ i)
		printf(" %d:%.4lf", ++ index, x.Yhaar[i]);

	for (int i = 0; i < HAAR; ++ i)
		printf(" %d:%.4lf", ++ index, x.Zhaar[i]);
